...
public object ThemeExpander {
    ...
    internal fun parseHexToArgb(hex: String): Int {
        ...
        // Optimization: Manual character iteration avoids the allocation overhead
        // and regex engine costs associated with String.substring() and toLong(16).
        var rgb = 0
        for (i in 1..6) {
            val c = hex[i]
            val v = when (c) {
    ...
    public fun argbToHex(argb: Int): String {
        // Optimization: CharArray.concatToString and manual bitwise shifting
        // avoids the substantial overhead of toString(16), padStart, and uppercase().
        val chars = CharArray(7)
        chars[0] = '#'
        val hexChars = "0123456789ABCDEF"
        chars[1] = hexChars[(argb ushr 20) and 0xF]
        chars[2] = hexChars[(argb ushr 16) and 0xF]
        chars[3] = hexChars[(argb ushr 12) and 0xF]
        chars[4] = hexChars[(argb ushr 8) and 0xF]
        chars[5] = hexChars[(argb ushr 4) and 0xF]
        chars[6] = hexChars[argb and 0xF]
