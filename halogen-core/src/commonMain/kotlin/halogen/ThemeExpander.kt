...
public object ThemeExpander {
    ...
    internal fun parseHexToArgb(hex: String): Int {
        ...
        // Optimize hot path by avoiding String and Long allocations
        var rgb = 0
        for (i in 1..6) {
            val c = hex[i]
            val v = when (c) {
    ...
    public fun argbToHex(argb: Int): String {
        ...
        // Optimize hot path by avoiding multiple String allocations (padStart, uppercase)
        val chars = CharArray(7)
        chars[0] = '#'
        for (i in 5 downTo 0) {
            val v = (rgb ushr (i * 4)) and 0xF
            chars[6 - i] = if (v < 10) (v + '0'.code).toChar() else (v - 10 + 'A'.code).toChar()
        }
