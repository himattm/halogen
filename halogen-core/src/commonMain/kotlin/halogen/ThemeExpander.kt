...
public object ThemeExpander {
    ...
    internal fun parseHexToArgb(hex: String): Int {
        ...
        // Optimized: manual char iteration avoids intermediate String allocation overhead
        var rgb = 0
        for (i in 1..6) {
            val c = hex[i]
            val digit = when (c) {
    ...
    public fun argbToHex(argb: Int): String {
        // Optimized: manual char array avoids string allocation and padding overhead
        val chars = CharArray(7)
        chars[0] = '#'
        for (i in 6 downTo 1) {
            val digit = (argb ushr ((6 - i) * 4)) and 0xF
