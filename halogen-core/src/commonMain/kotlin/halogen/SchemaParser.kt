...
public object SchemaParser {
    ...
    private fun stripCodeFences(input: String): String {
        val startFence = "```"
        val firstIndex = input.indexOf(startFence)
        if (firstIndex == -1) return input

        var contentStart = firstIndex + startFence.length

        // Skip "json" if it appears right after the fence
        if (contentStart + 4 <= input.length && input.substring(contentStart, contentStart + 4).lowercase() == "json") {
            contentStart += 4
        }

        val nextFenceIndex = input.indexOf(startFence, contentStart)
        if (nextFenceIndex == -1) return input

        return input.substring(contentStart, nextFenceIndex).trim()
    }
