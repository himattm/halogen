...
public object SchemaParser {
    ...
    // Optimization: Replaced regex parsing with raw string extraction (indexOf/substring).
    // Regex matching with `[\s\S]*?` can cause severe backtracking and performance
    // overhead on large JSON/LLM outputs. Using direct index operations provides
    // predictable linear performance and zero backtracking overhead.
    private fun stripCodeFences(input: String): String {
        ...
        if (input.startsWith("json", contentStart)) {
            contentStart += 4
        }
