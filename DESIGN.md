...
- Single provider model: pick one provider (on-device or cloud) per engine instance
...
**Decision:** Themes are stored by string key: any developer-defined identifier (a route, a subreddit, a category, a brand name). The resolve chain is: memory cache -> remote themes -> LLM provider -> default. In-memory LRU cache with configurable max entries (default 20).
