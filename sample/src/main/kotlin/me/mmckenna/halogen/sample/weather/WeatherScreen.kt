...
@Composable
fun WeatherScreen(engine: HalogenEngine) {
    val viewModel: WeatherViewModel = viewModel(factory = WeatherViewModel.Companion.factory(engine))
    val uiState by viewModel.uiState.collectAsState()
    val weather = weatherConditions[uiState.selectedIndex]

    // Theme is applied at app level by HalogenDemoApp's HalogenTheme —
    // engine.resolve() updates activeTheme which flows up to the app wrapper
    Column(
        modifier = Modifier
            .fillMaxSize()
            ...
            ScrollableTabRow(
                ...
                weatherConditions.forEachIndexed { index, condition ->
                    Tab(
                        ...
                        onClick = { viewModel.selectWeather(index) },
                        text = {
            ...
            Column(
                ...
                // This card shows the theme is contextual
                Card(modifier = Modifier.fillMaxWidth()) {
                    Column(
                        modifier = Modifier.padding(16.dp),
                        verticalArrangement = Arrangement.spacedBy(8.dp),
                        ...
                        Text(
                            "Each weather condition generates a unique Material 3 theme " +
                                "using Halogen + Gemini Nano. Tap the tabs above to see the " +
