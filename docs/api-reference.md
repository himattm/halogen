...
Complete API surface for all four Halogen modules.
...
| Method | Description |
|--------|-------------|
| `resolve(key: String, hint: String? = null): HalogenResult` | Resolve a theme: cache -> server -> LLM -> default |
...
Halogen.Builder()
    ...
    .fallbackProvider(provider: HalogenLlmProvider)
    .serverProvider(provider: HalogenServerProvider)
