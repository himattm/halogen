...
!!! info "Device Requirements"
    Gemini Nano requires **Pixel 9+**, **Samsung Galaxy S24+**, or other devices with on-device Gemini support and a locked bootloader. On unsupported devices, `availability()` returns `UNAVAILABLE` — pair with a `fallbackProvider()` for graceful degradation.
...
You can also wire up a `HalogenServerProvider` so the engine fetches themes automatically during resolve:
...
// Server themes resolve automatically — no LLM needed
...
val engine = Halogen.Builder()
    ...
    .serverProvider { key -> api.theme(key) } // Server fallback
    .config(HalogenConfig.Vibrant)            // Brand preset
    .build()

// Server-driven hints with on-device generation and prefetching
