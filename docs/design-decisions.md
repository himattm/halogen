...
2. **Port to pure Kotlin** - Reimplement the core color math ([CAM16, HCT](https://material.io/blog/science-of-color-design), tonal palettes) in Kotlin `commonMain`. Works on all KMP targets.
...
A full [Material 3](https://m3.material.io) `ColorScheme` has 49 color roles. The LLM could generate all 49, or it could generate a small number of seed values that the library expands.
