 ...
 *
 * **Mutable parameters**: [temperature], [topK], and [topP] are mutable for UI
 * configuration. However, the underlying [generativeModel] is lazy-initialized,
 * so changes to these parameters won't take effect until the provider is
 * recreated. This is fine for use cases like the Playground, which recreate
 * providers when settings change.
 *
 ...
 * @param topP Controls nucleus sampling (cumulative probability threshold). Default null.
 *   Note: ML Kit's Prompt API may not support this parameter, so it's stored but not passed to the model.
 */
