...
Developer calls:  engine.resolve(key = "coffee", hint = "coffee")
               ...
               └─────────┬──────────┘
                    MISS  │
                          ▼
               ┌─────────────────────┐
               │  RemoteThemes?    │──── HIT ──▶ Write L1 + L2, return
               └─────────┬──────────┘
                    MISS  │
                          ▼
┌─────────────────────────────────────────────────────┐
...
3. **Remote themes take priority over LLM.** If you have a backend that defines pre-built themes, those are used first and cached locally. The LLM provider handles unknown contexts.
4. **The LLM is pluggable.** `HalogenLlmProvider` is an interface. Ship with Gemini Nano for on-device, or use OpenAI/Claude for cloud, or write your own.
...
### 3.5 Single Provider Model

The engine uses a single `HalogenLlmProvider`. Pick the provider that fits your use case:

```kotlin
val halogen = Halogen.Builder(context)
    .provider(GeminiNanoProvider(context))       // on-device
...
```
cache MISS → remoteThemes MISS →
    provider (Nano) → generate → parse → cache → apply
```
...
| Locked bootloader         | Required             | Use cloud provider instead       |
| Device support            | Pixel 9+, Samsung S24+| Use cloud provider instead      |
...
// ── With remote themes from your backend ──
val halogen = Halogen.Builder(context)
    .provider(GeminiNanoProvider(context))
    .remoteThemes(MyApiThemeProvider(api))
...
enum class ThemeSource {
    ...
    SERVER,   // fetched from HalogenRemoteThemes
...
engine.resolve(key, hint)
    ...
    └────────────────────────────────────────────┘
         │ MISS
         ▼
    ┌─ 3. Remote themes (optional) ───────────┐
    │   remoteThemes?.fetchTheme("coffee")      │
    │   HIT? → write L1 + L2, return             │
    └────────────────────────────────────────────┘
         │ MISS or no remote themes
         ▼
    ┌─ 4. LLM provider ──────────────────┐
    │   provider.availability() → READY?     │
    │     YES → generate(prompt) → parse/validate│
    │     NO  → return Unavailable           │
...
### 9.8 Remote Themes (Optional)

For apps like Reddit where the server might define pre-built themes for known entities:
...
class RedditThemeProvider(private val api: RedditApi) : HalogenRemoteThemes {
...
val halogen = Halogen.Builder(context)
    ...
    .remoteThemes(RedditThemeProvider(api))
    .build()
```
...
suspend fun resolve(key: String, hint: String?): HalogenResult {
    ...
    return coroutineScope {
        val job = launch {
            // ... cache → remoteThemes → LLM provider → default
        }
        resolveJobs[key] = job
        job.join()
        // ...
...
| Scenario                        | Behavior                                |
|--------------------------------|------------------------------------------|
| Provider available              | Generate with provider                    |
| Provider unavailable            | Use dev's default theme; `HalogenSettingsCard` shows "unavailable" state |
| Provider returns invalid JSON   | Retry once. If still fails, use default   |
...
│       ├── HalogenRemoteThemes.kt         # Optional remote themes interface
│       └── db/
...
│   └── HalogenRemoteThemes.kt            # interface
...
- [ ] `HalogenEngine` - resolve flow: cache → remoteThemes → provider → default
- [ ] `Halogen.Builder` - wires provider, cache, remoteThemes, defaults
- [ ] `HalogenRemoteThemes` interface (optional remote theme fetch)
...
- Provider status (Nano available/downloading/unavailable)
